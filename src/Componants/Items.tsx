    ...
    <div className="flex flex-col gap-3 w-full overflow-scroll">
      {items.length >= 2 && items.length <= 6 ? (
        ...
        <p className="text-center text-red-500">
          The items should be between 2 and 6 at most.
          {items.length > 6 && (
            <button
              className="mt-2 rounded-full size-8 border-none flex items-center justify-center bg-linear-to-tr from-fuchsia-500 to-purple-800 text-white"
              onClick={() => setItems(items.slice(0, 6))}
            >
